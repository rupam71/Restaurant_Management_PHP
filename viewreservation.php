...
$mysqli->query("DELETE  FROM reserve WHERE reserveId=$id") or die($mysqli->error);
...
<head>
  ...
  <title>Reservation | All Grill Restaurant</title>
</head>
<body>
  <!-- header starts -->
  <header>
...
<section class="contact">
  <div class="container">
    ...
    <div class="reservation_title text-center justify-content-center pt-0">
      <h1 class="title_g">All Reservations</h1>
      <img class="img-fluid title_decor_g"  src="images/headingDecor.png" alt="heading">
    </div>
    <?php 
    $mysqli=new mysqli($host,$user,"",$db) or die(mysqli_error($mysqli));
    $result= $mysqli->query("Select * From reserve ORDER BY `reserveDate` ASC , `reserveTime` ASC") or die($mysqli->error); 
    ?>
    <div class="row">
        ...
        <div class="table-responsive r_table">
          <table class="table table-bordered table-striped table-hover">
            ...
            <tbody>
              ...
              $name = $row['reserveName'];
              $email = $row['reserveEmail']; 
              $phoneNo = $row['reservePhone'];
              $member = $row['totalMember'];  
              ?>
              <tr>
                <td><?php echo $date; ?></td>
                <td><?php echo $time; ?></td>
                ...
                <td><a href="viewreservation.php?delete=<?php echo $reserveId;?>" onclick="removed()" class="btn btn-danger"  data-toggle="tooltip" data-html="true"  title="Delete" data-placement="bottom" ><i class="fa fa-trash-o" aria-hidden="true"></i></a></td>
              </tr>
              <?php  }  ?>
